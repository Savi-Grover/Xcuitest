...
//public class SignupFordrive extends ExtentReportsDemo {
	public class PaymentCardText extends BaseClass {
	...
	public void PaymentCardTextValidations() throws MalformedURLException {
		// run launch chrome
		try {
	       ...
	       //import page class and initialse obj of this all page class
			...
			FileInputStream fs = new FileInputStream("/Users/sgrover7/eclipse-workspace/ios/src/test/resources/InputFile/PaymentCardText.xlsx");
			...
			for (int j = 1; j <= rowNum; j++) {
				...
				for (int i = 1; i <= noOfsignups; i++) {
					...
					/*
					 * else if (tier.contains("2")) { pkgpg.pkg_tier2.click(); } else if
					 * (tier.contains("4")) { pkgpg.pkg_tier4.click(); }
					 */
					else  {
					...
					//check payment card text
					creditcardpg.cond7();
					
					// Click on Lets Go
					creditcardpg.button_letsGo.click();	
					
					//verify payment card texts
					creditcardpg.Verify_PaymentCard();
					//creditcardpg.create_password();
					
					//Check if status is pending pickup
					//dashbpg.Verify_pendingPickup();
					
					//check if debit card added is default
				//dashbpg.Verify_PrimaryCard();
